...
@Config
@Autonomous(name = "red3gate1ai")
public class redthreegate1ai21 extends DbzOpMode {
...
//                    .build();
            Path12 = f.pathBuilder()
                    .addPath(
                            new BezierCurve(
                                    new Pose(m(55.100), 80.400),
                                    new Pose(m(9.848), 58.313),
                                    new Pose(m(14.669), 16.569)
                            )
                    )
                    .setTangentHeadingInterpolation()
                    .build();
        ...
        switch (state) {
            ...
            case shoot5:
                ...
                if (shootStarted && statetimer.seconds() >= shootDelay + 0.1 && !turretBump) {
                    turretBump = true; bumpTimer.reset();
                }
                if (shootStarted && statetimer.seconds() >= shootDelay + shootDur) {
                    endshoot();
                    intake.setPower(1);
    ...
    private double getdesiredturretdeg() {
        ...
        switch (state) {
            ...
            case shoot3:  case followPath3:  case followPath4: base =  -81.5; break;
            case shoot13: case followPath13:                   base =  -93; break;
            case shoot11: case followPath11:                   base =  -57.5; break;
